...
use o3\module\mini\o3_css_compressor as o3_css_compressor;
use o3\module\mini\o3_js_compressor as o3_js_compressor;
	...
	*/		
	 ...
	 */		
  	public function css_cache() {
		...
		if ( count($css_files) > 0 )
			foreach ( $css_files as $filename ) {
				...
				if ( $index == $this->cache_uniqid_css ) {
					...
					$md5 = $_[2];
					if ( time() <= $time ) {
						if ( $md5 == $cache_name.'.css' ) {
							if ( time() <= $time ) {
								$cache_file_name = $basename;	
		...
		//no valid cache create one
		if ( !$valid_cache_found ) {
			$cache_file_name = $this->cache_uniqid_css.'-'.$expire.'-'.$cache_name.'.css';
			...
			if ( $this->minimize && $this->minimize_css )
				$content = o3_css_compressor::process( $content );
	 ...
	 */		
 	public function js_cache() {
		...
		//no valid cache create one
		if ( !$valid_cache_found ) {
			$cache_file_name = $this->cache_uniqid_js.'-'.$expire.'-'.$cache_name.'.js';
			...
			if ( $this->minimize && $this->minimize_js )
				$content = o3_js_compressor::process( $content );
