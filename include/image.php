	...
	if ( file_exists($source) && is_readable($source) ) {		
		...
		if ( !file_exists($destination) ) {			
			if ( is_imagic() ) {
				...
				$command = "\"".O3_IMAGE_MAGIC."\" \"".$source."\" -profile \"USWebCoatedSWOP.icc\" ".( $flags & O3_IMAGE_FLATTEN ? '-flatten' : '' )." ".( $flags & O3_IMAGE_CLIP ? '-clip' : '')." ".( $background !== '' ? '-background '.$background : '')." -quality ".intval($quality)."% -density ".$dpi." -resize \"".$width."x".$height.$size_flag."\" -profile \"sRGB.icc\" \"".$destination."\"";
